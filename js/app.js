...
createApp({
    setup() {
        ...
        const company = reactive({ 
            fantasia: '', logo: '', cnpj: '', email: '', phone: '', 
        ...
        const tempApp = reactive({ 
            clientId: '', date: '', time: '', 
            location: { bairro: '' }, 
            details: { entryFee: 0, balloonColors: '' }, 
            notes: '', selectedServices: [], checklist: [] 
        });

        // --- VARIÁVEIS DO PORTAL DO CLIENTE ---
        ...
        const targetProviderId = ref(null); // ID do decorador vindo do link
        ...
        // ============================================================
        onMounted(async () => {
            // Verifica link personalizado do cliente
            ...
            if (params.get('acesso') === 'cliente') {
                ...
                if (providerUid) {
                    ...
                    // Tenta carregar a logo do decorador antes mesmo do login
                    try {
            ...
            onAuthStateChanged(auth, async (u) => {
                ...
                if (u) {
                    ...
                    syncData();
                    // Carrega dados do usuário logado (Decorador)
                    try {
        ...
        const toNum = (v) => { 
            ...
            const c=String(v).replace(',','.').replace(/[^0-9.-]/g,''); 
        ...
        // ============================================================
        // 5. INTEGRAÇÃO FIREBASE (Leitura e Escrita)
        // ============================================================
        ...
        const sanitizeApp = (docSnapshot) => {
            ...
            return { 
                ...
                paymentProof: data.paymentProof || null, // Novo campo
                isNewUpdate: data.isNewUpdate || false   // Novo campo
            };
        };
        ...
        const handleClientAccess = async () => {
            ...
            try {
                ...
                // Filtro Multi-tenant: Se tiver decorador no link, busca só dele
                let qConstraints = [];
                if (targetProviderId.value) {
                    qConstraints.push(where("userId", "==", targetProviderId.value));
                }

                // Busca Cliente
                let q = query(collection(db, "clients"), where("cpf", "==", rawTerm), ...qConstraints);
                let snap = await getDocs(q);
                if (snap.empty && numericTerm.length === 11) { 
                    q = query(collection(db, "clients"), where("cpf", "==", formattedCPF), ...qConstraints); 
                    snap = await getDocs(q); 
                }
                if (snap.empty) { 
                    q = query(collection(db, "clients"), where("email", "==", rawTerm), ...qConstraints); 
                    snap = await getDocs(q); 
                }
                ...
                let appQueryConstraints = [where("clientId", "==", clientDoc.id)];
                if (targetProviderId.value) {
                    appQueryConstraints.push(where("userId", "==", targetProviderId.value));
                }

                const qApps = query(collection(db, "appointments"), ...appQueryConstraints);
                ...
                // Se não veio pelo link mas achou eventos, carrega a empresa do primeiro evento
        ...
        const clientApproveBudget = async (app) => {
            const { isConfirmed } = await Swal.fire({
                ...
                text: 'Ao confirmar, a data será reservada. Você poderá anexar o comprovante de pagamento em seguida.',
                icon: 'question',
            ...
            if (isConfirmed) {
                authLoading.value = true;
                try {
                    // Atualiza status e cria notificação
                    ...
                    if(idx !== -1) clientAppointments.value[idx].status = 'pending';
                    
                    await Swal.fire('Aprovado!', 'Orçamento aprovado. Agora anexe o comprovante.', 'success');
                } catch (e) {
            ...
            reader.onload = async (e) => {
                try {
                    ...
                    await updateDoc(doc(db, "appointments", app.id), {
                        ...
                        isNewUpdate: true, // Notifica o decorador
                        paymentDate: new Date().toISOString()
                    });
                    
                    const idx = clientAppointments.value.findIndex(a => a.id === app.id);
                    if(idx !== -1) clientAppointments.value[idx].paymentProof = base64Image;

                    Swal.fire('Enviado!', 'Comprovante enviado com sucesso.', 'success');
                } catch (err) {
        ...
        const copyClientLink = () => { 
            // Adiciona o UID do decorador ao link
            const url = `${window.location.origin}${window.location.pathname}?acesso=cliente&uid=${user.value.uid}`; 
            navigator.clipboard.writeText(url).then(() => Swal.fire('Copiado!', 'Link copiado.', 'success')); 
        };
        ...
        const saveAppointment = async () => { 
            const appData = { 
                ...
                status: 'pending',
                isNewUpdate: false // Reset flag se o decorador salvar
            }; 
        ...
        const saveAsBudget = async () => {
            ...
            showAppointmentModal.value = false; 
            Swal.fire('Orçamento Criado!', 'Ver na aba Orçamentos.', 'success');
        };
        ...
        const showReceipt = (app) => { 
            ...
            // Remove a notificação de "Novo" quando o decorador abre o recibo
            if (app.isNewUpdate) {
                updateDoc(doc(db, "appointments", app.id), { isNewUpdate: false });
                app.isNewUpdate = false;
            }
        };

        // --- OUTRAS AÇÕES CRUD (Clientes, Serviços, Despesas) ---
        ...
        // --- ASSINATURA E EXPORTS ---
        // (Código de assinatura mantido idêntico)
        ...
        const saveSignature = async () => { const canvas = document.getElementById('signature-pad'); const dataUrl = canvas.toDataURL(); authLoading.value = true; try { await updateDoc(doc(db, "appointments", signatureApp.value.id), { clientSignature: dataUrl }); const idx = clientAppointments.value.findIndex(a => a.id === signatureApp.value.id); if(idx !== -1) clientAppointments.value[idx].clientSignature = dataUrl; showSignatureModal.value = false; Swal.fire('Sucesso', 'Assinado!', 'success'); } catch (e) { Swal.fire('Erro', 'Não salvou.', 'error'); } finally { authLoading.value = false; } };
        ...
        const generateContractPDF = () => { 
            ...
            } else { y += 20; doc.setFontSize(8); doc.text("* Este documento é apenas um orçamento e não garante a reserva da data.", 105, y, {align: "center"}); }
        ...
        return {
            ...
            // --- NOVAS FUNCIONALIDADES EXPORTADAS ---
            copyClientLink,
            clientApproveBudget,
            uploadPaymentProof,
