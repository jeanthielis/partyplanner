...
createApp({
    setup() {
        ...
        // --- 2. ÁREA DO CLIENTE E ASSINATURA ---
        ...
        // --- 4. MODAIS E CONTROLES ---
        const showAppointmentModal = ref(false);
        const showClientModal = ref(false);
        const showServiceModal = ref(false);
        const showExpenseModal = ref(false);
        const showReceiptModal = ref(false); // Novo controle para o Recibo
        ...
        const next7DaysApps = computed(() => { const today = new Date(); today.setHours(0,0,0,0); const nextWeek = new Date(today); nextWeek.setDate(today.getDate() + 7); const todayStr = today.toISOString().split('T')[0]; const nextWeekStr = nextWeek.toISOString().split('T')[0]; return pendingAppointments.value.filter(a => a.date >= todayStr && a.date <= nextWeekStr).sort((a,b) => a.date.localeCompare(b.date)); });

        // --- LIFECYCLE ---
        onMounted(() => {
            const params = new URLSearchParams(window.location.search);
            if (params.get('client') === 'true' || params.get('acesso') === 'cliente') {
            ...
            onAuthStateChanged(auth, async (u) => {
                if (u) {
                    ...
                    if (!uDoc.exists()) await setDoc(doc(db, "users", u.uid), { email: u.email, role: 'user', createdAt: new Date().toISOString() });
                    if (uDoc.exists() && uDoc.data().companyConfig) Object.assign(company, uDoc.data().companyConfig);
                    loadDashboardData();
                    syncData();
                } else { 
                    user.value = null; 
                    if (view.value !== 'client-portal') view.value = 'dashboard';
                }
            });
        ...
        const handleClientAccess = async () => {
            ...
            } catch (e) { console.error(e); if(e.code === 'permission-denied') Swal.fire('Erro', 'Verifique as regras do Firebase.', 'error'); else Swal.fire('Acesso Negado', 'Dados não encontrados.', 'error'); } finally { authLoading.value = false; }
        };
        ...
        const saveSignature = async () => { const canvas = document.getElementById('signature-pad'); if (isCanvasBlank(canvas)) return Swal.fire('Ops', 'Assine antes de salvar.', 'warning'); const dataUrl = canvas.toDataURL(); authLoading.value = true; try { await updateDoc(doc(db, "appointments", signatureApp.value.id), { clientSignature: dataUrl }); const idx = clientAppointments.value.findIndex(a => a.id === signatureApp.value.id); if(idx !== -1) clientAppointments.value[idx].clientSignature = dataUrl; showSignatureModal.value = false; Swal.fire('Sucesso', 'Assinado!', 'success'); } catch (e) { console.error(e); Swal.fire('Erro', 'Não foi possível salvar.', 'error'); } finally { authLoading.value = false; } };

        // --- CARREGAMENTO DE DADOS ---
        const loadDashboardData = async () => { if (!user.value) return; isLoadingDashboard.value = true; try { const [year, month] = dashboardMonth.value.split('-'); const startStr = `${year}-${month}-01`; const endStr = `${year}-${month}-${new Date(year, month, 0).getDate()}`; const qApps = query(collection(db, "appointments"), where("userId", "==", user.value.uid), where("date", ">=", startStr), where("date", "<=", endStr)); const qExp = query(collection(db, "expenses"), where("userId", "==", user.value.uid), where("date", ">=", startStr), where("date", "<=", endStr)); const [snapApps, snapExp] = await Promise.all([getDocs(qApps), getDocs(qExp)]); dashboardData.appointments = snapApps.docs.map(sanitizeApp).filter(a => a.status !== 'cancelled'); dashboardData.expenses = snapExp.docs.map(sanitizeExpense); dashboardData.appointments.forEach(a => fetchClientToCache(a.clientId)); } catch (e) { console.error(e); } finally { isLoadingDashboard.value = false; } };
        ...
        const filteredListAppointments = computed(() => { let list = agendaTab.value === 'pending' ? pendingAppointments.value : historyList.value; if(clientSearchTerm.value) list = list.filter(a => getClientName(a.clientId).toLowerCase().includes(clientSearchTerm.value.toLowerCase())); return list.sort((a,b) => a.date.localeCompare(b.date)); });
        ...
        const handleAuth = async () => { if (!authForm.email || !authForm.password) return Swal.fire('Atenção', 'Preencha todos os campos.', 'warning'); authLoading.value = true; try { if (isRegistering.value) { const userCredential = await createUserWithEmailAndPassword(auth, authForm.email, authForm.password); const newUser = userCredential.user; await updateProfile(newUser, { displayName: authForm.name }); await setDoc(doc(db, "users", newUser.uid), { email: authForm.email, role: 'user', createdAt: new Date().toISOString(), companyConfig: { fantasia: authForm.name || 'Minha Empresa', logo: '', cnpj: '', email: authForm.email, phone: '', rua: '', bairro: '', cidade: '', estado: '' } }); await Swal.fire({ title: 'Sucesso!', text: 'Conta criada com sucesso!', icon: 'success', timer: 2000, showConfirmButton: false }); } else { await signInWithEmailAndPassword(auth, authForm.email, authForm.password); const Toast = Swal.mixin({ toast: true, position: 'top-end', showConfirmButton: false, timer: 3000, timerProgressBar: true }); Toast.fire({ icon: 'success', title: 'Login realizado com sucesso' }); } } catch (error) { console.error(error); Swal.fire('Ops!', 'Erro de login.', 'error'); } finally { authLoading.value = false; } };
        ...
        // --- EXIBIÇÃO DE RECIBO (AGORA USANDO MODAL) ---
        ...
        return {
            ...
            currentReceipt, showReceipt, showReceiptModal, // NOVO EXPORT
