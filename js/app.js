...
createApp({
    setup() {
        ...
        // --- COMPUTEDS (DEFINIÇÃO DAS VARIÁVEIS DO HTML) ---
        
        // 1. Definição explícita da variável que estava faltando
        ...
        const checklistProgress = (app) => { if (!app || !app.checklist || !Array.isArray(app.checklist) || app.checklist.length === 0) return 0; const done = app.checklist.filter(t => t.done).length; return Math.round((done / app.checklist.length) * 100); };
