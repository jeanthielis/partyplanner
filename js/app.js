    ...
    setup() {
        ...
        const handleClientAccess = async () => {
            ...
            try {
                ...
                if (snap.empty && numericTerm.length === 11) { q = query(collection(db, "clients"), where("cpf", "==", formattedCPF), ...qConstraints); snap = await getDocs(q); }
        ...
        const downloadClientReceipt = async (app) => { 
            if (!app.clientSignature) return Swal.fire('Atenção', 'Assine o contrato antes de baixar.', 'warning');
