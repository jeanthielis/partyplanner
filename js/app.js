...
// IMPORTANDO AS NOVAS FUNÇÕES DO FIREBASE.JS
import { 
    db, auth, 
...
createApp({
    setup() {
        ...
        const handleChangePassword = async () => {
            ...
            const { value: formValues } = await Swal.fire({
                ...
                preConfirm: () => {
                    ...
                    if (!current || !newP) { Swal.showValidationMessage('Preencha os dois campos'); }
                    return [current, newP];
                }
            });
            ...
            if (formValues) {
                ...
                try {
                    // 1. Reautenticar (Segurança do Firebase exige isso)
                    ...
                    if (error.code === 'auth/invalid-credential') msg = 'Senha atual incorreta.';
                    if (error.code === 'auth/weak-password') msg = 'A nova senha é muito fraca (min 6 caracteres).';
        ...
        return {
            ...
            handleChangePassword // <--- Função exportada aqui
        };
    }
}).mount('#app');
